...
function buildResultSummary(strengths, risks, meta) {
  ...
  return `${strengthLead}${riskLead}${topRisk.empathy}那些“我明明知道，但就是会这样”的地方，其实已经有线索了。${secondRisk.cost}${balanceLead}${responseNote}可以先从一个很小的动作开始：${topRisk.microSteps[0]}。`;
}
...
function renderResponsePattern(info) {
  ...
  const body = info.level === "veryHigh"
    ? "你不是简单地没有主见，而是对很多题都能想到例外、对象和情境差异。只有当这种犹豫非常明显时，才会进入“雾里校准的银莲花”。这些迟疑更像一张触发地图：谁、什么压力、哪类评价最容易让你反复校准自己。"
    : info.level === "high"
    ? "这通常说明你的性格表现不是固定模式，而是会随对象、关系和压力强度变化。能意识到“我不确定”本身就是一种觉察，不需要急着把自己归类。"
    : info.level === "medium"
      ? "有些题可能对你来说不是绝对像或不像。这个犹豫值得被保留，后续更适合结合具体事件复盘。"
      : "这说明你对多数题目的自我判断比较清楚；如果看到不舒服的地方，也先把它当成信息，不急着否定自己。";
...
function renderDeepAnalysis(meta) {
  ...
  const pressureLine = meta.responseInfo.level === "veryHigh"
    ...
    : meta.responseInfo.level === "medium"
      ? "你有一部分答案并不绝对，说明你在不同对象面前会有不同版本。最有价值的不是总分，而是那些让你迟疑的题。"
